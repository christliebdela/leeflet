...
-- Project Modules (sub-areas within a project, e.g. Auth, Admin, Billing)
CREATE TABLE IF NOT EXISTS public.project_modules (
    ...
    created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL,
    CONSTRAINT unique_project_module_name UNIQUE (project_id, name)
);
...
-- Migration support: rename project_components -> project_modules and items.component_id -> items.module_id if upgrading
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'project_components')
     AND NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'project_modules') THEN
    ALTER TABLE public.project_components RENAME TO project_modules;
  END IF;

  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'items' AND column_name = 'component_id')
     AND NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'items' AND column_name = 'module_id') THEN
    ALTER TABLE public.items RENAME COLUMN component_id TO module_id;
  ELSIF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'items' AND column_name = 'module_id') THEN
...
-- Compatibility view for old queries / clients (enforcing querying user's RLS)
...
DO $$ BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.project_modules; EXCEPTION WHEN duplicate_object THEN null; END $$;
