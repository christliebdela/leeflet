    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.createdCount > 0 || result.updatedCount > 0) {
          ...
          toast.success('GitHub issues are up to date');
        }
      } else {
        toast.error(result.error || 'Failed to sync GitHub issues');
      }
    } catch (err: any) {
      toast.error(err.message || 'Error syncing GitHub issues');
    } finally {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5 min-w-0 mr-1.5 overflow-hidden" data-tauri-drag-region>
        ...
        {/* GitHub Linked Repository & Sync Issues Button */}
        {viewMode.type === 'project' && activeProj?.githubRepo && (() => {
          const parsed = parseGitHubRepo(activeProj.githubRepo);
          const repoLabel = parsed ? `${parsed.owner}/${parsed.repo}` : activeProj.githubRepo;
