...
import { ArrowUpCircle, ArrowDown, RefreshCw, X, CheckCircle2, AlertTriangle } from 'lucide-react';
...
function renderFormattedReleaseNotes(text: string) {
  ...
  return lines.map((line, idx) => {
    ...
    if (trimmed.startsWith('### ')) {
      return (
        <h4 key={idx} className="text-xs font-bold text-[#111827] dark:text-white pt-1 pb-0.5 tracking-tight">
          {trimmed.replace(/^###\s+/, '')}
        </h4>
      );
    }
    ...
    if (trimmed.startsWith('## ')) {
      return (
        <h3 key={idx} className="text-xs font-bold text-[#111827] dark:text-white pt-1 pb-0.5 tracking-tight">
          {trimmed.replace(/^##\s+/, '')}
        </h3>
      );
    }
    ...
    if (trimmed.startsWith('- ') || trimmed.startsWith('* ')) {
      ...
      return (
        <div key={idx} className="flex items-start gap-2 text-xs text-[#4b5563] dark:text-[#d4d4d8] leading-relaxed">
...
export const UpdateModal: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      >
        {/* Header Ribbon */}
        <div className="px-5 pt-5 pb-4 border-b border-[#f3f4f6] dark:border-[#27272a] flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-[#f4f5f6] dark:bg-[#202024] border border-[#e5e7eb] dark:border-[#2e2e33] flex items-center justify-center text-[#111827] dark:text-white shrink-0 shadow-2xs">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-1.5 py-0.5 rounded-[4px] bg-[#f4f5f6] dark:bg-[#27272a] text-[#111827] dark:text-white font-mono text-[10px] font-semibold border border-[#e5e7eb] dark:border-[#3f3f46]">
              ...
              <p className="text-xs text-[#6b7280] dark:text-[#a1a1aa] mt-0.5">
                ...
                {releaseDate ? ` • Released on ${releaseDate}` : ' • Ready to install'}
              </p>
            </div>
          </div>
        ...
        <div className="p-5 flex-1 max-h-64 overflow-y-auto custom-scrollbar space-y-2 text-xs leading-relaxed text-[#374151] dark:text-[#d4d4d8]">
          <div className="font-semibold text-[#111827] dark:text-white text-xs mb-1">
            What's in this version:
          </div>

          <div className="p-3 rounded-[8px] bg-[#f9fafb] dark:bg-[#141416] border border-[#e5e7eb] dark:border-[#27272a] space-y-2 select-text">
            {releaseNotes
              ? renderFormattedReleaseNotes(releaseNotes)
              : 'Various performance improvements, stability fixes, and design enhancements.'}
          </div>
          ...
          {isDownloaded && (
            <div className="flex items-center gap-2 p-2.5 rounded-[6px] bg-[#f4f5f6] dark:bg-[#202024] border border-[#e5e7eb] dark:border-[#2e2e33] text-[#111827] dark:text-white text-xs font-medium">
              ...
              <span>Update downloaded successfully! Click restart to apply changes.</span>
            </div>
          )}
        </div>

        {/* Footer Action Buttons */}
        <div className="px-5 py-3.5 bg-[#fafafa] dark:bg-[#141416] border-t border-[#f3f4f6] dark:border-[#27272a] flex items-center justify-end gap-2.5">
          <button
            type="button"
            onClick={() => setModalOpen(false)}
          ...
          {isDownloaded ? (
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-1.5 bg-[#111827] dark:bg-white hover:bg-[#1f2937] dark:hover:bg-[#e4e4e7] text-white dark:text-[#111827] rounded-[6px] text-xs font-semibold transition-all shadow-subtle cursor-pointer active:scale-[0.98]"
            >
            ...
            <button
              ...
              disabled={isDownloading}
              className="flex items-center gap-1.5 px-4 py-1.5 bg-[#111827] dark:bg-white hover:bg-[#1f2937] dark:hover:bg-[#e4e4e7] disabled:opacity-50 text-white dark:text-[#111827] rounded-[6px] text-xs font-semibold transition-all shadow-subtle cursor-pointer active:scale-[0.98]"
            >
